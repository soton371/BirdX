...
def sendPasswordSmtp(recipientMail: str):
    try:
        ...
        smtpServer = smtplib.SMTP(config.settings.smtp_host, config.settings.smtp_port)
        smtpServer.starttls()
        smtpServer.login(config.settings.sender_email,config.settings.send_otp_password)
        msg = EmailMessage()
...
def createAccessToken(email: str):
    expire = datetime.now(utilities.my_timezone) + timedelta(days=ACCESS_TOKEN_EXPIRE_DAY)
    return jwt.encode({"email": email, "exp": expire}, SECRET_KEY, algorithm=ALGORITHM)

def createRefreshToken(email: str):
    expire = datetime.now(utilities.my_timezone) + timedelta(days=REFRESH_TOKEN_EXPIRE_DAY)
    return jwt.encode({"email": email, "exp": expire}, SECRET_KEY, algorithm=ALGORITHM)
...
def getCurrentUser(token: str = Depends(oauth2_scheme), db: Session = Depends(database.get_db)):
    ...
    user=db.query(auth_model.Admin).filter(auth_model.Admin.email == verifyToken.email).first()
